...
#ifdef SSD1306_INCLUDE_FONT_16x15
    ssd1306_SetCursor(4, 4);
    ssd1306_WriteString("Hello! Neha", Font_16x15, White);
    ssd1306_SetCursor(4, 24);
    ssd1306_WriteString("Hello! Shivam", Font_16x15, White);
...
void scrollTextSeamless(char *text, uint8_t y_pos, uint8_t speed) {
    int16_t x = 0;
    int16_t text_width = strlen(text) * Font_7x10.width;

    while (1) {
        ssd1306_Fill(Black);

        for (int16_t offset = x; offset < SSD1306_WIDTH; offset += text_width) {
            ssd1306_SetCursor(offset, y_pos);
            ssd1306_WriteString(text, Font_7x10, White);
        }

        for (int16_t offset = x - text_width; offset > -text_width; offset -= text_width) {
            ssd1306_SetCursor(offset, y_pos);
            ssd1306_WriteString(text, Font_7x10, White);
        }
        ...
        x -= speed;                     // ← move by speed pixels per frame
        if (x <= -text_width) {
            x = 0;
        }
